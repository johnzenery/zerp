...
Public Class frm_sales_view_quotation
    ...
    Private Sub btn_approved_Click(sender As Object, e As EventArgs) Handles btn_approved.Click
        Try
            If MsgBox("Press 'Yes' to continue", MsgBoxStyle.YesNo Or MsgBoxStyle.Information, "Confirmation") = MsgBoxResult.Yes Then
                Using conn
                    conn.Open()
                    Using cmd = New MySqlCommand("UPDATE ims_quotations SET status='Approved', approved_by=@approved_by, approved_date=CURRENT_TIMESTAMP WHERE quotation_id=@id", conn)
    ...
    Private Sub btn_delete_Click(sender As Object, e As EventArgs) Handles btn_delete.Click
        If MsgBox("Do you want to delete this quotation?", vbQuestion + vbYesNo, "Confirmation") = vbYes Then
            Try
                conn.Open()
                Dim cmd = New MySqlCommand("UPDATE ims_quotations SET is_deleted='1' WHERE quotation_id=" & lbl_id.Text, conn)
                cmd.ExecuteNonQuery()
    ...
    Private Sub btn_transfer_Click(sender As Object, e As EventArgs) Handles btn_transfer.Click
        ...
        If SaveData("Approved") Then
            ...
            If radio_new.Checked = True Then
                ...
                Try
                    Using conn
                        conn.Open()
                        Dim cmd = New MySqlCommand("SELECT IF( EXISTS( SELECT * FROM ims_customers WHERE first_name = @customer), 1, 0) AS res", conn)
                ...
                If is_new_customer = 1 Or is_existing_customer = 1 Then
                    ...
                    Try
                        Using conn
                            conn.Open()
                            Dim cmd = New MySqlCommand("UPDATE ims_quotations SET customer_type='existing' WHERE quotation_id=" & lbl_id.Text, conn)
                            cmd.ExecuteNonQuery()
                        End Using
                    Catch ex As Exception
                        MsgBox(ex.Message, vbCritical, "Error")
            ...
            With frm
                ...
                frm.cb_tax_applied.Checked = cb_tax_applied.Checked
                ...
                If Not String.IsNullOrEmpty(txt_delivery_fee.Text) Then
                    ...
                    frm.lbl_delivery_fee.LinkColor = Color.Red
                    frm.lbl_delivery_fee.Text = "Remove Delivery Fee"
                End If
            End With
        ...
        '    frm.Show()
        '    frm.cbb_customer.Text = cbb_customer.Text
        '    frm.txt_priv_notes.Text = txt_priv_notes.Text
        '    frm.txt_pub_notes.Text = txt_priv_notes.Text
        '    frm.txt_quote_id.Text = lbl_quote_id.Text.Replace("Quotation #", "Q")
    ...
    Private Sub cbb_customer_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbb_customer.SelectedIndexChanged
        ...
        Try
            ...
            While rdr.Read
                ...
                txt_delivery_address.Text = IIf(IsDBNull(rdr("address")), "", rdr("address"))
