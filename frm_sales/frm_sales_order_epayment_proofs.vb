...
Public Class frm_sales_order_epayment_proofs
    ...
    Public Sub LoadData()
        Try
            Using conn
                conn.Open()
                Dim cmd = New MySqlCommand("SELECT proof_gallery FROM ims_orders WHERE order_id=@id", conn)
                cmd.Parameters.AddWithValue("@id", CInt(lbl_order_id.Text.Replace("SO", "")))
    ...
    Private Sub btn_save_Click(sender As Object, e As EventArgs) Handles btn_save.Click
        ...
        Try
            Using conn
                conn.Open()
                Dim cmd = New MySqlCommand("UPDATE ims_orders SET proof_gallery=@proof_gallery, payment_status=@status WHERE order_id=@id", conn)
                cmd.Parameters.AddWithValue("@id", CInt(lbl_order_id.Text.Replace("SO", "")))
                ...
                If cmd.ExecuteNonQuery() Then MsgBox("Success!", vbInformation, "Information")
                Me.Close()
