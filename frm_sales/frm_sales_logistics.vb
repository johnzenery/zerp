...
Public Class frm_sales_logistics
    ...
    Private Sub Load_logistics()
        Try
            conn.Open()
            Using cmd = New MySqlCommand("SELECT * FROM ims_logistics WHERE is_deleted='0'", conn)
                cmd.ExecuteNonQuery()
    ...
    Private Sub btn_delete_ButtonClick(sender As Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles btn_delete.ButtonClick
        ...
        Try
            conn.Open()
            Using cmd = New MySqlCommand("UPDATE ims_logistics SET is_deleted='1' WHERE logistics_id=@id", conn)
