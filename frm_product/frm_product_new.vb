...
Public Class frm_product_new
    ...
    Private Sub Load_Main_Categories()
        Try
            conn.Open()
            Dim cmd = New MySqlCommand("SELECT main_category FROM `ims_inventory` GROUP BY main_category", conn)
    ...
    Private Sub Load_Sub_Categories()
        Try
            conn.Open()
            Dim cmd = New MySqlCommand("SELECT sub_category FROM `ims_inventory` GROUP BY sub_category", conn)
            Dim rdr As MySqlDataReader = cmd.ExecuteReader
    ...
    Private Sub Load_Brands()
        Try
            conn.Open()
            Dim cmd = New MySqlCommand("SELECT brand FROM `ims_inventory` GROUP BY brand", conn)
            Dim rdr As MySqlDataReader = cmd.ExecuteReader
    ...
    Private Sub NewProduct()
        Try
            conn.Open()
            Dim duplicate = New MySqlCommand("SELECT COUNT(*) FROM `ims_inventory` WHERE winmodel=@model or description=@description or pid=@pid", conn)
            ...
            Dim cmd = New MySqlCommand("INSERT INTO ims_inventory (pid, winmodel, supmodel, supplier, description, technical_description, hazards, main_category, sub_category, brand, tags, barcode, serialized, base_price, discount, cost, regular_price, dealer_price, vip_price, length, width, height, weight, qty_per_box, ideal_stock, warning_stock, stock_duration, status, warranty_period, warranty_coverage, date_entry)
                                        VALUES (@pid, @winmodel, @supmodel, (SELECT id FROM ims_suppliers WHERE supplier=@supplier), @description, @technical_description, @hazards, @main_category, @sub_category, @brand, @tags, @barcode, @serialized, @base_price, @discount, @cost, @regular_price, @dealer_price, @vip_price, @length, @width, @height, @weight, @qty_per_box, @ideal_stock, @warning_stock, @stock_duration, @status, @warranty_period, @warranty_coverage, @date_entry)", conn)
            ...
            If inserted >= 1 Then MsgBox("Successfully Added!", vbInformation + vbOKOnly, "Information")

            'ClearFields()
            frm_main.LoadFrm(New frm_product_new)
    ...
    'Update Product
    Private Sub UpdateProduct(pid As String, src As String)
        Try
            conn.Open()

            If Not txt_pid.Text.Equals(old_pid) Then
                If check_duplicate(conn, "pid") Then
            ...
            If Not txt_winmodel.Text.Equals(old_model) Then
                If check_duplicate(conn, "winmodel") Then
            ...
            If Not txt_description.Text.Equals(old_description) Then
                If check_duplicate(conn, "description") Then
            ...
            Dim cmd = New MySqlCommand("UPDATE ims_inventory SET pid=@pid, winmodel=@winmodel, supmodel=@supmodel, supplier=(SELECT id FROM ims_suppliers WHERE supplier=@supplier), description=@description, technical_description=@technical_description,
                                ...
                                stock_duration=@stock_duration, status=@status, warranty_period=@warranty_period, warranty_coverage=@warranty_coverage WHERE pid=" & pid, conn)
            ...
            Select Case src
                Case "masterlist" : frm_main.LoadFrm(New frm_product_catalogue)
                Case "approval" : frm_main.LoadFrm(New frm_admin_approval)
            End Select
