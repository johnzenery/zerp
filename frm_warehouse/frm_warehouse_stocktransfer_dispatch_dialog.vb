...
Public Class frm_warehouse_stocktransfer_dispatch_dialog
    ...
    'Print Sub Transfer
    Public Sub printSubTransfer(stid As Integer)
        ...
        Try
            ...
            Using conn = New MySqlConnection(str)
                ...
                Using cmd = New MySqlCommand("SELECT sub_transfer_id,
                            root_transfer_id, ims_stock_transfer_sub.transfer_type, 
                            SRC.store_name AS src_store, RECVR.store_name AS recvr_store, 
                            ims_stock_transfer_sub.item, ims_stock_transfer_sub.total_cbm, ims_stock_transfer_sub.total_kgs,
                    ...
                    'Set to DataTable
                    SetDataTable(items, table.transferred_units, conn)
    ...
    'Set to DataTable
    Private Sub SetDataTable(units As String, dt As DataTable, conn As MySqlConnection)
        ...
        If Not String.IsNullOrEmpty(units) Then
            ...
            Using cmd = New MySqlCommand("SELECT ims_inventory.pid, winmodel, description, qty_per_box, masterbox_qty,
                ...
                For i = 0 To comma.Length - 1
                    ...
                    Using rdr = cmd.ExecuteReader
                        While rdr.Read
                            dt.Rows.Add(equal(1).Trim.Replace(";", ""), rdr("qty_per_box"), rdr("masterbox_qty"), rdr("winmodel"), rdr("description"), "")
                        End While
                    End Using
    ...
    Private Sub btn_print_Click(sender As Object, e As EventArgs) Handles btn_print.Click
        printSubTransfer(txt_transfer_id.Text.Replace("ST", ""))
    End Sub
End Class