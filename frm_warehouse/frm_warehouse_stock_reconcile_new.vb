...
Public Class frm_warehouse_stock_reconcile_new
    ...
    Private Sub load_warehouse()
        Try
            Using conn
                conn.Open()
                Dim mySqlCommand = New MySqlCommand("SELECT store_name FROM ims_stores", conn)
            ...
            cbb_stores.SelectedText = frm_main.user_store.Text
    ...
    Private Sub load_model()
        Try
            ...
            Using conn
                conn.Open()
                Dim cmd = New MySqlCommand("SELECT winmodel FROM ims_inventory INNER JOIN " & my_store & " AS MyStore ON MyStore.pid=ims_inventory.pid", conn)
            ...
            Using conn
                ...
                If status = "Approved" Then
                    ...
                    Using cmd = New MySqlCommand(query, conn)
                        ...
                        If ans > 0 Then
                            Using cmd_update_status = New MySqlCommand("UPDATE ims_reconciled_stocks SET status=@status, approved_by=@approved_by WHERE id=@id", conn)
                                cmd_update_status.Parameters.AddWithValue("@id", CInt(lbl_title.Text.Replace("Reconcile Request #", "")))
                                cmd_update_status.Parameters.AddWithValue("@status", status)
                                cmd_update_status.Parameters.AddWithValue("@approved_by", frm_main.user_id.Text)
                                cmd_update_status.ExecuteNonQuery()

                                MsgBox("Request has been Approved!", vbInformation, "Declined")
                ...
                ElseIf status = "Declined" Then
                    ...
                    MsgBox("Request has been Declined!", vbInformation, "Declined")
                    frm_main.LoadFrm(New frm_admin_approval)
                    Me.Close()
                End If
    ...
    Private Sub cbb_model_EditValueChanged(sender As Object, e As EventArgs) Handles cbb_model.EditValueChanged
        Try
            ...
            Using conn
                conn.Open()
                Dim cmd = New MySqlCommand("SELECT pid, description FROM ims_inventory WHERE winmodel=@model", conn)
        ...
        Try
            Using conn
                ...
                Dim cmd = New MySqlCommand("INSERT INTO ims_reconciled_stocks (src_store, qty, item, reconciled_type, reconciled_by, notes, status, created_at)
                            VALUES ((SELECT store_id FROM ims_stores WHERE store_name=@store_name), @qty, @item, @reconciled_type, @reconciled_by, @notes, @status, CURRENT_TIMESTAMP)", conn)
                ...
                cmd.Parameters.AddWithValue("@reconciled_by", frm_main.user_id.Text)
