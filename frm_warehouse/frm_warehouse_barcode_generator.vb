    ...
    Private Sub btn_generate_Click(sender As Object, e As EventArgs) Handles btn_generate.Click
        ...
        If rb_per_unit.Checked Then 'IF PER MODEL
            ...
            If txt_qty.Value = 0 Then
                ...
                Return
            End If
            If cbb_printers.SelectedIndex = -1 Then
                MsgBox("No selected printer.", vbExclamation, "No Printer")
                Return
            End If
...
#Region "Methods"
    ...
    Private Sub Print_barcode_per_item()
        ...
        Try
            ...
            Using conn = New MySqlConnection(str)
                conn.Open()
                Using cmd = New MySqlCommand("SELECT IFNULL(barcode, pid) AS barcode_no, winmodel 
                                    FROM ims_inventory
            ...
            Using engine As New Engine(True)
                ...
                'Create Path
                Dim dir_path = Application.StartupPath & "\tmp"
                ...
                Dim btFormat As LabelFormatDocument = engine.Documents.Open(full_path, cbb_printers.SelectedItem.ToString())

                'Write Values to 
                Dim tmpFile = dir_path & "\barcodes_to_print.csv"
                File.WriteAllText(tmpFile, Units)
                Dim tf = New TextFile("barcode_template")
                tf.FileName = tmpFile
    ...
    Public Sub Print_barcode_per_receipt()
        ...
        Using conn = New MySqlConnection(str)
            conn.Open()
            Using cmd = New MySqlCommand("SELECT IFNULL(inv.barcode, inv.pid) AS barcode_no, inv.winmodel, qty
                                    FROM ims_deliveries
