...
Public Class frm_warehouse_stocktransfer_request_new
            ...
            Using connection = New MySqlConnection(str)
                ...
                Using cmd = New MySqlCommand("SELECT " & StoreQuery & " ims_inventory.pid, winmodel, description, qty_per_box, masterbox_qty, IFNULL(inv.qty, 0) AS qty,
                    ...
                    For i = 0 To row.Length - 2
                        ...
                        For j = 8 To grid_request_stock.Columns.Count - 1
                            If j Mod 2 > 0 Then
                                grid_request_stock.Rows(i).Cells(5).Value += grid_request_stock.Rows(i).Cells(j).Value
                            End If
                        Next
    ...
    Public Sub LoadEdit(stid As Integer)
        ...
        Me.Text = "Edit Stock Transfer - Request"
    ...
    Private Sub SetTotals()
        ...
        For i = 0 To grid_request_stock.Rows.Count - 1
            For j = 8 To grid_request_stock.Columns.Count - 1
                If j Mod 2 > 0 Then
                    total_cbm += grid_request_stock.Rows(i).Cells(6).Value * IIf(IsDBNull(grid_request_stock.Rows(i).Cells(j).Value), 0, grid_request_stock.Rows(i).Cells(j).Value)
                    total_kgs += grid_request_stock.Rows(i).Cells(7).Value * IIf(IsDBNull(grid_request_stock.Rows(i).Cells(j).Value), 0, grid_request_stock.Rows(i).Cells(j).Value)
                End If
            Next
        Next
    ...
    Private Sub grid_request_stock_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs) Handles grid_request_stock.CellEndEdit
        If Not String.IsNullOrEmpty(cbb_src_warehouse.Text) Then
            ...
            ElseIf e.ColumnIndex > 8 Then
                ...
                For i = 8 To grid_request_stock.Columns.Count - 1
                    If i Mod 2 > 0 Then
                        grid_request_stock.Rows(e.RowIndex).Cells(5).Value += IIf(IsDBNull(grid_request_stock.Rows(e.RowIndex).Cells(i).Value), 0, grid_request_stock.Rows(e.RowIndex).Cells(i).Value)
                    End If
                Next
