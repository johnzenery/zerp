...
Public Class frm_warehouse_PackageManagement_packed
    ...
    Private Sub btn_print_Click(sender As Object, e As EventArgs) Handles btn_print.Click
        Dim sub_transfer_id = CInt(txt_sub_transfer_id.Text)

        Using connect = New MySqlConnection(str)
            connect.Open()

            Dim frm = New frm_warehouse_stocktransfer_dispatch_dialog
            frm.printSubTransfer(sub_transfer_id)

            'Print Transfer Slip
            'If txt_transfer_type.Text.Equals("Distribute") Then
            '    frm.print_transfer_distribute(transfer_id, connect)

            'ElseIf txt_transfer_type.Text.Equals("Request") Then
            '    frm.print_transfer_request(transfer_id, connect)
    ...
    Private Sub btn_packed_Click(sender As Object, e As EventArgs) Handles btn_packed.Click
        ...
        If txt_status.Text.Equals("Arranging") Then
            ...
            'Validate
            If cbb_packer.SelectedIndex = -1 Or String.IsNullOrEmpty(txt_prepared_by.Text) Then
            ...
            If MsgBox("Press 'Yes' to confirm.", vbInformation + vbYesNo, "Confirmation") = vbYes Then
                Try
                    Using conn = New MySqlConnection(str)
                        conn.Open()
                        Dim sub_transfer_id As Integer = txt_sub_transfer_id.Text
                        ...
                        If sub_transfer_function(sub_transfer_id, conn) = True Then
                            ...
                            Dim cmd = New MySqlCommand("UPDATE ims_stock_transfer_sub
                                    SET checked_by=@packer, prepared_by=@prepared_by, date_packed=CURRENT_TIMESTAMP, status='Packed'
            ...
            If MsgBox("Press 'Yes' to confirm.", vbInformation + vbYesNo, "Confirmation") = vbYes Then
                Try
                    Using conn = New MySqlConnection(str)
                        ...
                        If cmd.ExecuteNonQuery() > 0 Then
                            MsgBox("UNPACKED!", vbInformation, "PACKED")
                            Me.Close()
                        End If
    ...
    Private Sub LoadPackers()
        Try
            Using conn = New MySqlConnection(str)
                ...
                While rdr.Read
                    cbb_packer.Properties.Items.Add(rdr("first_name"))
                End While
