        ...
        Try
            Using conn
                ...
                Dim cmd = New MySqlCommand("SELECT concat('PO',LPAD(purchase_id,5,0)) as poid, ims_suppliers.supplier FROM ims_purchase
                                            LEFT JOIN ims_suppliers ON ims_purchase.supplier=ims_suppliers.id WHERE deleted='0' AND is_payment_first='1'", conn)
                cmd.ExecuteNonQuery()
    ...
    Private Sub grid_po_view_FocusedRowChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs) Handles grid_po_view.FocusedRowChanged
        ...
        Try
            Using conn
                ...
                Dim cmd = New MySqlCommand("SELECT * FROM ims_purchase WHERE purchase_id=@id", conn)
    ...
    Private Sub btn_create_Click(sender As Object, e As EventArgs) Handles btn_create.Click
        ...
        Try
            Using conn
                conn.Open()
                'CHECK IF EXIST
                Using cmd = New MySqlCommand("SELECT COUNT(*) FROM ims_delivery_receipts WHERE receipt_ref=@ref", conn)
                ...
                Dim payables_cmd = New MySqlCommand("INSERT INTO ims_delivery_receipts (supplier_id, receipt_ref, receipt_type, amount, discount, status, received_date, store_id, purchase_id)
		                VALUES ((SELECT id FROM ims_suppliers WHERE supplier=@supplier), 
                                @receipt_ref, @receipt_type, @amount, @discount, @status, CURRENT_TIMESTAMP, (SELECT store_id FROM ims_stores WHERE store_name=@store_name), @purchase_id)", conn)
                ...
                payables_cmd.Parameters.AddWithValue("@purchase_id", CInt(txt_poid.Text.Replace("PO", "")))
                payables_cmd.ExecuteNonQuery()

                MsgBox("New payable was created!", vbInformation, "Information")
                Me.Close()
                frm_main.LoadFrm(New frm_accounting_payables)
