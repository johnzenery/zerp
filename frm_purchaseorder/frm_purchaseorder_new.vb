...
Public Class frm_purchaseorder_new
    ...
    Private Sub load_locations()
        ...
        Try
            conn.Open()
            Dim rdr = New MySqlCommand("SELECT store_name FROM `ims_stores`", conn).ExecuteReader
    ...
    Private Sub SaveData(status As String)
        ...
        Try
            ...
            cmd.Parameters.AddWithValue("@user_id", frm_main.user_id.Text)
            cmd.ExecuteNonQuery()
    ...
    'Breadcrumb
    Private Sub link_po_list_Click(sender As Object, e As EventArgs) Handles link_po_list.Click
        Me.Close()
